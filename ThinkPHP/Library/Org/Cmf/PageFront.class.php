...
class PageFront {
    ...
    private function UrlParameters($url = array()) {
        ...
        if ($this->Static) {
			...
			if(MODULE_NAME == 'Home' && CONTROLLER_NAME == 'Category') { //分类重写
				if(strpos($request_url, '?') !== false) {
					$fixed_url = preg_replace('/^(.*?_\d+).*$/', '\\1', $request_url);
//					$fixed_url = strstr($request_url, '?', true);
				} else {
					$fixed_url = preg_replace('/(\/\d+\.html|\.html)$/', '', $request_url);
				}
//					echo $fixed_url;
				return $fixed_url.'/*.html';
			} else if(MODULE_NAME == 'Home' && CONTROLLER_NAME == 'Index') { //分类重写
				return '/p/*.html';
			}
    ...
    public function Pager($Page_tpl = '') {
		...
		if(MODULE_NAME == 'Home' && CONTROLLER_NAME == 'Index') { //首页分页链接加nofollow
        ...
        for ($i = $pStart; $i <= $pEnd; $i++) {
            if ($i == $cfg ['pageindex']) {
                ...
                if ($this->Static && $i == 1) {
					$pList .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . $this->PageLink['index'] . '"> ' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                } else {
                    $pList .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $i, $cfg ['link']) . '"> ' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                }
            }
        }
        if ($cfg ['listsidelong'] > 0) {
            if ($cfg ['listsidelong'] < $pStart) {
                for ($i = 1; $i <= $cfg ['listsidelong']; $i++) {
					if ($this->Static && $i == 1) {
						$pListStart .= '<a '.$nofollow.' href="' . $this->PageLink['index'] . '">' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                     } else {
                        $pListStart .= '<a '.$nofollow.' href="' . str_replace('*', $i, $cfg ['link']) . '">' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                    }
                }
                ...
                if ($cfg ['listsidelong'] >= $pStart && $pStart > 1) {
                    for ($i = 1; $i <= ($pStart - 1); $i++) {
                        if ($this->Static && $i == 1) {
                            $pListStart .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . $this->PageLink['index'] . '"> ' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                        } else {
                            $pListStart .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $i, $cfg ['link']) . '"> ' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                        }
                    }
                }
            }
            if (($cfg ['pagecount'] - $cfg ['listsidelong']) > $pEnd) {
                ...
                for ($i = (($cfg ['pagecount'] - $cfg ['listsidelong']) + 1); $i <= $cfg ['pagecount']; $i++) {
					if ($this->Static && $i == 1) {
						$pListEnd .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . $this->PageLink['index'] . '">' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                     } else {
                        $pListEnd .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $i, $cfg ['link']) . '"> ' . str_replace('*', $i, $cfg ['list']) . ' </a> '.$this->linkwraper_after;
                    }
                }
            } else {
                if (($cfg ['pagecount'] - $cfg ['listsidelong']) <= $pEnd && $pEnd < $cfg ['pagecount']) {
                    for ($i = ($pEnd + 1); $i <= $cfg ['pagecount']; $i++) {
						if ($this->Static && $i == 1) {
						    $pListEnd .= '<a '.$nofollow.' href="' . $this->PageLink['index'] . '">' . str_replace('*', $i, $cfg ['list']) . '</a> '.$this->linkwraper_after;
                        } else {
                            $pListEnd .= $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $i, $cfg ['link']) . '"> ' . str_replace('*', $i, $cfg ['list']) . ' </a> '.$this->linkwraper_after;
                        }
                    }
        ...
        if ($cfg ['pageindex'] > 1) {
            if ($this->Static) {
                $pFirst = $this->linkwraper_pre.'<a '.$nofollow.' href="' . $this->PageLink['index'] . '">' . $cfg ['first'] . '</a> '.$this->linkwraper_after; //首页
            } else {
                $pFirst = $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', 1, $cfg ['link']) . '">' . $cfg ['first'] . '</a> '.$this->linkwraper_after; //首页
            }
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a '.$nofollow.' href="' . $this->PageLink['index'] . '">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $cfg ['pageindex'] - 1, $cfg ['link']) . '" class="prev">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after;
            }
        }
        //下一页，尾页
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
            $pLast = $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $cfg ['pagecount'], $cfg ['link']) . '">' . $cfg ['last'] . '</a> '.$this->linkwraper_after;
            $pNext = $this->linkwraper_pre.'<a '.$nofollow.' href="' . str_replace('*', $cfg ['pageindex'] + 1, $cfg ['link']) . '" class="next">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
        }
